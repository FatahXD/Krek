...
\t|_______/[___]   \'-;__/[_____][_____]'.__.'[___]    	
			{m}•{k}•{h}•{sir} Author : FinoDafaresta {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def menu(my_name,my_id):
	...
	cetak(nel('                            \tWARNING \n•Selamat Datang Di Script Al-Vino \n•Jangan Lupa Follow Github Al-Vino \n•Jangan Recode Tanpa Seizin Author \n•Jangan Memperjual Belikan Script \n•Selamat Mencoba'))
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/Ｆｉｎｏ-DUMP')
	except FileNotFoundError:
...
def passwrd():
	...
	print(f'>> Mainkan Modpes Setiap {m}20ID😁{x} Idz\n')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rProcess {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
