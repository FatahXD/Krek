...
def banner():
	os.system('clear')
	baz_bann(f'''{asu}
d8b   db d8888b. d8888b.  .d8b.  
888o  88 88  `8D 88  `8D d8' `8b 
88V8o 88 88   88 88oobY' 88ooo88 
88 V8o88 88   88 88`8b   88~~~88 
88  V888 88  .8D 88 `88. 88   88 
VP   V8P Y8888D' 88   YD YP   YP 
...
def menu(my_name, my_id):
    ...
    print(f'{xxx} 2. KELUAR {B}[ HAPUS KOOKIE ]{xxx}')
    print('╰────────────────────')
...
def nge_krek():
    try:
        ...
        exit()
    try:
        dwi = int(input(f"(xxx)└──C BERAPA ID YG MAU DI CRACK:{B}  "))
    except ValueError:
        exit()
...
def atur_dulu():
	...
	print(' 1. METODH VALID')
	metod = input(f'{xxx}└──C KETIK 1 : ')
	print(f'╭──────────────────────────────────────')
	print(f'└──C {h}GAUSAH NGELUH PEPEK PAKE AJA')
	if metod in ['1','01']:
		baz.append('metod1')
	else:
		baz.append('metod1')
...
def metod1(idf,pwv,awal):
	...
	sys.stdout.write(f" NDRAXP{B} [{kun} {(loop)}{puti}/{hijo}{len(id)}{puti} ] [ {hijo}{(ok)}{xxx}/{kun}{(cp)}{xxx} ] \r");sys.stdout.flush()
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict():
				...
				tree.add(f"⌲ Tahun pembuatan: {mer}{tahun(idf)}{puti}")
